	...
	private void getBooksFromRequest (HttpServletRequest request) {
		...
		if (categoryString != null && !categoryString.isEmpty()) {
			if (categoryString.equalsIgnoreCase(ON_SPECIAL_CMD)) {
				list = bookDatabaseAccessor.getSpecialBooks();
				request.getSession().setAttribute(CATEGORY_CMD, "On Special");
			}
				...
				if (category.equals(Category.ScienceFiction)) {
					request.getSession().setAttribute(CATEGORY_CMD, "Science Fiction");
				} else {
					request.getSession().setAttribute(CATEGORY_CMD, categoryString);
				}
			}
		}
			...
			request.getSession().setAttribute(CATEGORY_CMD, "Search Results");
		}
